import { ConfirmChannel, Connection, Options, Replies } from 'amqplib';
...
export class RxConfirmChannel {
     ...
     * @returns boolean
     */
    public publish(exchange: string, routingKey: string, content: Buffer, options?: Options.Publish) {
        console.log('confirm publish');
        const func = (_e: string, _r: string, _b: Buffer, _o: Options.Publish, cb: any) => {
            this.channel.publish(_e, _r, _b, _o, cb);
        };
        const bound = Rx.Observable.bindNodeCallback(func);
